...
| スキル             | 対象                                             | 概要                                                                                                                                                                                                                      |
| ------------------ | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `gdscript-quality` | Godot 4.x（typed Dictionary は 4.4+ を自動判別） | 静的型付けを徹底した production 品質の GDScript を書くためのスキル<br>- 型注釈・typed collections・UNTYPED*/UNSAFE* 警告の除去・命名/構成規約・node/signal パターンをカバー<br>- 同梱 `check.sh` で headless 検証まで行う |

### handover / takeover — セッション間の作業引き継ぎ

| スキル     | 役割       | 概要                                                                                                                                                                                                 |
| ---------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `handover` | 書き出し側 | セッションの作業コンテキスト（タスク状態・次の手順・設計判断・関連ファイル・git 状態）を handover ファイルに上書き保存<br>- 保存先はデフォルト `docs/handover.md`、環境変数 `HANDOVER_FILE` で変更可 |
| `takeover` | 再開側     | handover ファイルを読んでコンテキストを復元<br>- 現在の git 状態と照合し、乖離があればユーザーに確認してから再開<br>- 却下済み案の再提案を抑止                                                       |
